...
require(ggplot2)
require(scales)
data = read.csv("/Users/jaya_kuppuswamy/Documents/Jaya/team-datasciencegit/employee_retention_data.csv", header=TRUE, stringsAsFactors = TRUE) #read data set
...
#who quit was offered in the next job. Otherwise, things like: promotions or raises received during the employee tenure 
#would be interesting.
#2. The major findings are that employees quit at year anniversaries or at the beginning of the year.
# Both cases make sense. Even if you don't like your current job, you often stay for 1 yr before quitting + you often get stocks 
# after 1 yr so it makes sense to wait. Also, the beginning of the year is well known to be the best time to change job: 
#companies are hiring more and you often want to stay until end of Dec to get the calendar year bonus.
#3. Employees with low and high salaries are less likely to quit. Probably because employees with high salaries are happy there 
#and employees with low salaries are not that marketable, so they have a hard time finding a new job.
...
str(data_surv)
summary(data_surv)
data_surv$time<-difftime(mdy(data_surv$quit_date),mdy(data_surv$join_date),"days")

data_surv$event<-ifelse(is.na(data_surv$time),0,1)
kmsurvival <- survfit(Surv(time,event) ~ 1 , data_surv)  
summary(kmsurvival)
plot(kmsurvival, xlab="Time", ylab="Survival Rate of Employees")
ggsurvplot(kmsurvival, xlab="Time", ylab="Survival Rate of Employees")


X <- cbind(data_surv$dept  , data_surv$seniority , data_surv$salary_percentile)
group <- data_surv$dept # can do this with dept
str(X)
length(data_surv$seniority)


# Kaplan-Meier non-parametric analysis by group
kmsurvival <- survfit(Surv(time,event) ~ 1 , data_surv)  
kmsurvival1 <- survfit(Surv(time,event) ~ group,data_surv)
summary(kmsurvival1)
plot(kmsurvival1, xlab="Time", ylab="Survival Rate of Employees")
ggsurvplot(kmsurvival1, xlab="Time", ylab="Survival Rate of Employees",risk.table = T)



## Conclusion
## Dept does not seem to impact quitting of the job. However, engineers seem to change jobs more
